...
void Transparency::initShaders() {
    ...
    m_alphaTestingShader->attachVertexShaderFromFile(
        getAssetFullPath(alphaTestingVsRelPath), version);
    m_alphaTestingShader->attachFragmentShaderFromFile(
    ...
    m_alphaBlendingShader->attachVertexShaderFromFile(
        getAssetFullPath(alphaBlendingVsRelPath), version);
    m_alphaBlendingShader->attachFragmentShaderFromFile(
    ...
    m_depthPeelingInitShader->attachVertexShaderFromFile(
        getAssetFullPath(oitInitVsRelPath), version);
    m_depthPeelingInitShader->attachFragmentShaderFromFile(
        getAssetFullPath(oitInitFsRelPath), version);
    m_depthPeelingInitShader->link();

    m_depthPeelingShader.reset(new GLSLProgram);
    m_depthPeelingShader->attachVertexShaderFromFile(getAssetFullPath(oitPeelVsRelPath), version);
    m_depthPeelingShader->attachFragmentShaderFromFile(getAssetFullPath(oitPeelFsRelPath), version);
    m_depthPeelingShader->link();

    m_depthPeelingBlendShader.reset(new GLSLProgram);
    m_depthPeelingBlendShader->attachVertexShaderFromFile(
        getAssetFullPath(oitBlendVsRelPath), version);
    m_depthPeelingBlendShader->attachFragmentShaderFromFile(
        getAssetFullPath(oitBlendFsRelPath), version);
    m_depthPeelingBlendShader->link();

    m_depthPeelingFinalShader.reset(new GLSLProgram);
    m_depthPeelingFinalShader->attachVertexShaderFromFile(
        getAssetFullPath(oitFinalVsRelPath), version);
    m_depthPeelingFinalShader->attachFragmentShaderFromFile(
...
void Transparency::initDepthPeelingResources() {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        m_depthTextures[i].reset(new Texture2D(
            GL_DEPTH_COMPONENT32F, m_windowWidth, m_windowHeight, GL_DEPTH_COMPONENT, GL_FLOAT));
#else
        m_depthTextures[i].reset(new Texture2D(
