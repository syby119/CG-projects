...
void PostProcessing::initGeometryPassResources() {
    m_gPosition.reset(new Texture2D(GL_RGB32F, m_windowWidth, m_windowHeight, GL_RGB, GL_FLOAT));
    m_gPosition->bind();
    ...
    m_gNormal.reset(new Texture2D(GL_RGB32F, m_windowWidth, m_windowHeight, GL_RGB, GL_FLOAT));
    m_gNormal->bind();
    ...
    m_gAlbedo.reset(new Texture2D(GL_RGB32F, m_windowWidth, m_windowHeight, GL_RGB, GL_FLOAT));
    m_gAlbedo->bind();
    ...
    m_gBufferShader->attachVertexShaderFromFile(getAssetFullPath(geometryVsRelPath));
    m_gBufferShader->attachFragmentShaderFromFile(getAssetFullPath(geometryFsRelPath));
...
void PostProcessing::initSSAOPassResources() {
    ...
    m_ssaoShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_ssaoShader->attachFragmentShaderFromFile(getAssetFullPath(ssaoFsRelPath));
    m_ssaoShader->link();

    m_ssaoBlurShader.reset(new GLSLProgram);
    m_ssaoBlurShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_ssaoBlurShader->attachFragmentShaderFromFile(getAssetFullPath(ssaoBlurFsRelPath));
    m_ssaoBlurShader->link();

    // TODO: modify ssao_lighting.frag
    m_ssaoLightingShader.reset(new GLSLProgram);
    m_ssaoLightingShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_ssaoLightingShader->attachFragmentShaderFromFile(getAssetFullPath(ssaoLightingFsRelPath));
...
void PostProcessing::initBloomPassResources() {
    ...
    m_lightShader->attachVertexShaderFromFile(getAssetFullPath(lightVsRelPath));
    m_lightShader->attachFragmentShaderFromFile(getAssetFullPath(lightFsRelPath));
    m_lightShader->link();

    // TODO: modify extract_bright_color.frag
    m_brightColorShader.reset(new GLSLProgram);
    m_brightColorShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_brightColorShader->attachFragmentShaderFromFile(getAssetFullPath(brightColorFsRelPath));
    m_brightColorShader->link();

    // TODO: modify gaussian_blur.frag
    m_blurShader.reset(new GLSLProgram);
    m_blurShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_blurShader->attachFragmentShaderFromFile(getAssetFullPath(gaussianBlurFsRelPath));
    m_blurShader->link();

    m_blendShader.reset(new GLSLProgram);
    m_blendShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_blendShader->attachFragmentShaderFromFile(getAssetFullPath(blendBloomMapFsRelPath));
    m_blendShader->link();
}

void PostProcessing::initShaders() {
    m_drawScreenShader.reset(new GLSLProgram);
    m_drawScreenShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_drawScreenShader->attachFragmentShaderFromFile(getAssetFullPath(quadFsRelPath));
