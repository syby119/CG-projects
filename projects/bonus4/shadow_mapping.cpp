...
void ShadowMapping::initShaders() {
    ...
    m_lambertShader->attachVertexShaderFromFile(getAssetFullPath(lambertVsRelPath));
    m_lambertShader->attachFragmentShaderFromFile(getAssetFullPath(lambertFsRelPath));
    ...
    m_quadShader->attachVertexShaderFromFile(getAssetFullPath(quadVsRelPath));
    m_quadShader->attachFragmentShaderFromFile(getAssetFullPath(quadFsRelPath));
    m_quadShader->link();

    // 2. cube shader for point light depth visualization
    m_cubeShader.reset(new GLSLProgram);
    m_cubeShader->attachVertexShaderFromFile(getAssetFullPath(cubeVsRelPath));
    m_cubeShader->attachFragmentShaderFromFile(getAssetFullPath(cubeFsRelPath));
    m_cubeShader->link();

    // 3. quad cascade shader for directional light cascade depth visualization
    m_quadCascadeShader.reset(new GLSLProgram);
    m_quadCascadeShader->attachVertexShaderFromFile(getAssetFullPath(quadCsmVsRelPath));
    m_quadCascadeShader->attachFragmentShaderFromFile(getAssetFullPath(quadCsmFsRelPath));
...
void ShadowMapping::initDepthResources() {
    ...
    m_depthTexture.reset(new Texture2D(
        GL_DEPTH_COMPONENT, shadowMapResolution, shadowMapResolution, GL_DEPTH_COMPONENT,
        GL_FLOAT));
    m_depthTexture->bind();
