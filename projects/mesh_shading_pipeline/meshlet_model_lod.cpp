...
MeshletModelLod::MeshletModelLod(const std::vector<std::string>& filepaths) {
    for (size_t i = 0; i < filepaths.size(); ++i) {
        ...
        if (!tinyobj::LoadObj(
                &attrib, &shapes, &materials, &warn, &err, filepaths[i].c_str(),
                mtlBaseDir.c_str())) {
        ...
        const uint32_t primitiveIndicesOffset{
            static_cast<uint32_t>(m_combinedPrimitiveIndices.size())};
        ...
        for (size_t i = 0; i < meshlets.size(); ++i) {
            m_combinedMeshlets.push_back(
                {meshlets[i].vertex_count, meshlets[i].vertex_offset + vertexIndicesOffset,
                 meshlets[i].triangle_count, meshlets[i].triangle_offset + primitiveIndicesOffset});
        }

        std::copy(
            primitiveIndices.begin(), primitiveIndices.end(),
