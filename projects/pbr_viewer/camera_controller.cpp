...
void CameraController::update(const Input& input, float deltaTime) {
    ...
    bool mouseMove = (mouse.move.xNow - mouse.move.xOld != 0) || 
                     (mouse.move.yNow - mouse.move.yOld != 0);
...
glm::vec2 CameraController::getMouseOnScreen(float pageX, float pageY) {
    return glm::vec2((pageX - _screenLeft) / _screenWidth,
        (pageY - _screenTop) / _screenHeight);
}

glm::vec2 CameraController::getMouseOnCircle(float pageX, float pageY) {
    return glm::vec2((2.0f * (pageX - _screenLeft) - _screenWidth) / _screenWidth,
...
void CameraController::rotateCamera() {
    ...
    if (angle > 0) {
        ...
        glm::vec3 objectSidewaysDirection = glm::normalize(
            glm::cross(objectUpDirection, eyeDirection));
...
void CameraController::rotateCamera1() {
    ...
    _eye = radius * glm::vec3(std::sin(theta) * std::sin(phi), std::cos(phi),
...
void CameraController::panCamera() {
    ...
    if (glm::length(mouseChange) > 0) {
        ...
        glm::vec3 pan = mouseChange.x * glm::normalize(glm::cross(_eye, cameraUp)) + 
                        mouseChange.y * cameraUp;
