...
public:
    ...
    shaderc_include_result* GetInclude(
        const char* includeFilepath, shaderc_include_type type, const char* shaderFilepath,
...
static int reflectUniformVarRecursive(
    spirv_cross::Compiler const& compiler, spirv_cross::SPIRType const& type,
    std::string const& name, int location, GLProgram::UniformVarInfoMap& uniformVarInfos) {
    ...
    if (type.array.empty()) {
        for (size_t i = 0; i < type.member_types.size(); ++i) {
            ...
            location = details::reflectUniformVarRecursive(
                compiler, memberType, name + "." + memberName, location, uniformVarInfos);
        }
    } else {
        // handle array / array of array
        std::vector<uint32_t> indices(type.array.size(), 0);
        do {
            ...
            for (size_t i = 0; i < type.member_types.size(); ++i) {
                ...
                location = details::reflectUniformVarRecursive(
                    compiler, memberType, nameIndexed + "." + memberName, location,
                    uniformVarInfos);
            }
        } while (details::getNextIndices(type.array, indices));
    }
...
static GLProgram::TextureInfoMap reflectTextureInfos(
    ...
    for (auto const& sampledImage : sampledImages) {
        ...
        if (mask.get(spv::DecorationBinding)) {
            ...
            std::cerr << "Should vulkan workflow to specify texture binding, instead of location"
                      << std::endl;
        }
...
std::shared_ptr<GLProgram> ProgramManager::create(std::vector<ShaderSource> const& shaderSources) {
    ...
    for (auto& shaderSource : shaderSources) {
        ...
        auto preprocessResult{
            compiler.PreprocessGlsl(code, kind, (char*)inputFileName.c_str(), options)};
        ...
        compileResult =
            compiler.CompileGlslToSpv(code, kind, (char*)inputFileName.c_str(), options);
        ...
        shaderModules.emplace_back(
            spirvRuntime, shaderSource.stage, shaderSource.entrypoint.c_str());
...
void ProgramManager::printSpirvReflection(std::vector<uint32_t> const& spirv) {
    ...
    auto printResources = [](spirv_cross::CompilerGLSL& compiler, const std::string& tag,
        ...
        for (auto& res : resources) {
            ...
            bool isBlock{
                compiler.get_decoration_bitset(type.self).get(spv::DecorationBlock)
            ...
            bool isSizedBlock{
                ...
                && (compiler.get_storage_class(res.id) == spv::StorageClass::StorageClassUniform
                    || compiler.get_storage_class(res.id)
                           == spv::StorageClass::StorageClassUniformConstant)};
            ...
            spirv_cross::ID fallback_id{
                !isPushConstant && isBlock ? spirv_cross::ID(res.base_type_id)
                                           : spirv_cross::ID(res.id)};
            ...
            spirv_cross::Bitset mask{
                isSSBO ? compiler.get_buffer_block_flags(res.id)
                       : compiler.get_decoration_bitset(res.id)};

            std::string array;
            for (auto arr : type.array) {
                array = spirv_cross::join("[", arr ? spirv_cross::convert_to_string(arr) : "", "]")
                        + array;
            }

            printf(
                " ID %03u : %s%s", uint32_t(res.id),
                !res.name.empty() ? res.name.c_str()
                                  : compiler.get_fallback_name(fallback_id).c_str(),
                array.c_str());

            if (mask.get(spv::DecorationLocation)) {
                printf(
                    " (Location : %u)", compiler.get_decoration(res.id, spv::DecorationLocation));
            }

            if (mask.get(spv::DecorationDescriptorSet)) {
                printf(
                    " (Set : %u)", compiler.get_decoration(res.id, spv::DecorationDescriptorSet));
            }
            ...
            if (static_cast<const spirv_cross::CompilerGLSL&>(compiler)
                    .variable_is_depth_or_compare(res.id)) {
                printf(" (comparison)");
            }
            ...
            for (size_t i = 0; i < type.member_types.size(); ++i) {
                ...
                if (memberType.basetype != spirv_cross::SPIRType::Struct) {
                    printf(
                        "    %s%dx%d %s", details::getTypeStr(memberType).c_str(),
                        memberType.vecsize, memberType.columns, memberName.c_str());
                ...
                std::cout << "      memberType.array.size(): " << memberType.array.size()
                          << std::endl;
            }
        }
        printf("\n");
...
bool ProgramManager::readFile(
    std::filesystem::path const& filepath, std::string& content) noexcept {
    try {
        std::ifstream fp;
